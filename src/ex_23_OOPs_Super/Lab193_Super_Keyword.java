package ex_23_OOPs_Super;

public class Lab193_Super_Keyword {
    public static void main(String[] args) {
        Car c1 = new Car(100);
        c1.display();
    }

}
class Vehicle{
    public int maxSpeed = 180;

    Vehicle() { // default constructor
        System.out.println("DC");
    }
    Vehicle(int a) {
        System.out.println("Parameterised Constructor");
    }
    // Method Overloading - Same, same name function with different arguments.
    void message(int a) {
        System.out.println("Type 2");
    }

    void message() {
        System.out.println("Type 1");
    }

    int message(String a) {
        System.out.println("Type 4");
        return 0;
    }

    void display() {
        System.out.println("Vehicle Parent");
    }
}

class Car extends Vehicle{
    private int maxSpeed = 281;

    //Right click -> generate getter & setter

    public int getMaxSpeed() {
        return maxSpeed;
    }

    public void setMaxSpeed(int maxSpeed) {
        this.maxSpeed = maxSpeed;
    }
    Car(){
        super(); // calling parents default
    }
    Car(int a){
        super(10); // parent's parameterized call

    }

    @Override
    void display() {
        System.out.println(super.maxSpeed); //  instance variable call - Parents variable
        System.out.println(this.maxSpeed); // This means my variable call.- My variable
        System.out.println("Hi Override Function");
    }
}